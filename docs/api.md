...
Moon Bridge 对外暴露 API 兼容 OpenAI Responses API，代理请求并转换为 Anthropic Messages API 格式发送至上游 Provider。
...
| 字段 | 类型 | 说明 |
|------|------|------|
| `model` | `string` | 必需。会通过 `provider.models` 映射为上游模型名 |
...
- `usage.input_tokens`：累加 Anthropic 的 `input_tokens` + `cache_creation_input_tokens` + `cache_read_input_tokens`。
