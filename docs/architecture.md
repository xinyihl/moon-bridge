...
- 流式：设置 SSE 头后立即下发 `BuildPreambleEvents()` 生成的 commentary 消息（`phase: "commentary"`，文本 "Collecting from upstream..."），让客户端在上游收集期间看到活动状态；随后调用 Provider `StreamMessage()` → 收集所有 SSE 事件 → `Bridge.ConvertStreamEventsWithContext()` 批量转换（通过 `StreamOptions` 跳过重复的 `response.created`/`response.in_progress`、偏移序列号和输出索引）→ 写入 SSE 流。Commentary 消息带 `phase: "commentary"` 标记，Codex 在 history compaction 和 thread forking 时会自动过滤，不会混入后续请求上下文。
...
- **`ConvertStreamEventsWithContext()`**：逐事件将 Anthropic SSE 流转为 OpenAI 流。管理 content_block 级别的 state 跟踪，处理 text / tool_use / server_tool_use 三种 block 类型的流式拼接。支持 `StreamOptions` 参数：当已发送 preamble 时可跳过 `response.created`/`response.in_progress`、偏移序列号和输出索引、覆盖 response ID。
- **`BuildPreambleEvents()`**：生成 commentary 消息的 SSE 事件序列（`phase: "commentary"`，文本 "Collecting from upstream..."），在上游收集前立即下发给客户端，让 Codex TUI 显示 "Working" 状态。返回 `PreambleResult` 包含事件列表、response ID、序列号偏移和输出项数量，供后续 `StreamOptions` 使用。
