...
- `/v1/models` 端点返回 Codex `ModelsResponse` 格式（`{"models": [...]}`），仅包含 route 别名。每个 `ModelInfo` 包含 Codex 反序列化所需的全部字段（`slug`、`shell_type`、`visibility`、`truncation_policy`、`supported_reasoning_levels` 等），元数据从 provider model catalog 继承。
- 错误处理分两层：
...
- `deepseek_v4`：按当前模型路由到的 Provider 配置启用，处理 reasoning_content 剥离、reasoning_effort → thinking 映射、流式 thinking 跟踪等 DeepSeek 特有行为。
