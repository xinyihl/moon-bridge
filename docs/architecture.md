...
Moon Bridge 是一个 Go 中间层服务，提供 OpenAI Responses API 兼容接口。Transform 模式下，它会按模型别名路由请求：Anthropic 协议 Provider 走 OpenAI Responses ↔ Anthropic Messages 转换，`openai-response` Provider 走 Responses 直通代理。
...
- `protocol: "openai-response"`：保留 OpenAI Responses 格式，直接透传到上游 Responses 端点，并把请求里的模型别名改写为上游真实模型名。
...
flowchart LR
    ...
    openai_proxy["OpenAI Responses passthrough\nPOST /v1/responses"]
...
透明代理模式。请求按 OpenAI Responses 协议原样转发至上游 Responses Provider，不做协议转换。用于抓取 Codex 原生 Responses 请求作为协议对齐基准。
...
- `extension/codex` 依赖 `internal/foundation/openai`、`internal/protocol/anthropic`、`internal/foundation/config` 等底层包。
- `extension/codex` **不依赖** `internal/protocol/bridge` 或 `internal/service/server`，杜绝循环依赖。
- `bridge` 和 `server` 反向调用 `extension/codex` 提供的纯函数或 codec。
